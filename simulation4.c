...
void char2uint16_t(unsigned char *in, uint16_t *out, size_t out_size, unsigned int unit_size) {
	unsigned int mask = pow(2, unit_size) - 1;
	for (size_t i=0; i<out_size; i++)
		out[i] = (uint16_t)((*(unsigned int*)(&in[i*unit_size/8])) >> i*unit_size%8) & mask;
}

void char2ulong(unsigned char *in, unsigned long *out, size_t out_size, unsigned int unit_size) {
...
int main(int argc, char **argv)
{
	...
	uint16_t exp_q_16[m];
	char2uint16_t(q, exp_q_16, m, p_size);
	...
	//Correctness check. Can be removed////////////////////////////////////////
	for (j=0; j<m; j++){
	...
	char2ulong(dec_sec, exp_dec_sec_long, m, p_size+n_size);
	for (i=0; i<m; i++)
		exp_dec_sec_long[i] %= p;

	// 3.expand agg_q to agg_q_16
	uint16_t agg_q_16[m];
	char2uint16_t(agg_q, agg_q_16, m, p_size);
...
void *sub_enc(void *vargp) {
	...
	printf("thread:%u before_loop_time:%lf loop_time:%lf start%u= end=%u\n", myid, print_time(&t1, &t2), print_time(&t2, &t3), start, end);
}
